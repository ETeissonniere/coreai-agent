...
struct ThinkTagParserTests {
    ...
    @Test("Pre-opened reasoning prefix — content before closing marker is reasoning")
    func startsInsideThinking() {
        var parser = ThinkTagParser(startsInsideThinking: true)
        let events = parser.consume("hidden work</think>Final answer") + parser.flush()
        #expect(eventStrings(events, kind: .reasoning) == ["hidden work"])
        #expect(eventStrings(events, kind: .text) == ["Final answer"])
    }
