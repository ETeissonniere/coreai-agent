...
public struct CoreAILanguageModel: LanguageModel {
    ...
    public var kvCacheStatistics: KVCacheStatistics { engine.kvCacheStatistics }
    ...
    public struct CoreAIExecutor: LanguageModelExecutor {
        ...
        public nonisolated(nonsending) func respond(
            ...
            let reasoningMode = Self.reasoningMode(
                for: transcript,
        ...
        private func respondVanilla(
            ...
            // Decode the complete generated suffix on each step. Tokenizer
            // whitespace is contextual: decoding only the previous/current
            // token pair can erase paragraph breaks and spaces at token
            // boundaries. The response is capped to a small output budget, so
            // correctness here is worth the bounded re-decode cost.
            var generatedTokens: [Int32] = []
            var emittedDecodedText: String = ""
            var tokenStep: Int = 0
            ...
            var thinkParser = ThinkTagParser(
                ...
                // Qwen3-family assistant prefixes already end in `<think>`, so
                // generated tokens begin inside the reasoning channel.
                startsInsideThinking: modelIdentifier.lowercased().contains("qwen3")
                    && !modelIdentifier.lowercased().contains("title")
            ...
            for try await output in tokenStream {
                ...
                let decodedText = tokenizer.decode(tokens: generatedTokens.map { Int($0) })
                decodeSpan.end()

                let common = decodedText.commonPrefix(with: emittedDecodedText)
                ...
                if hasReplacementChar {
                    ...
                    // buffer; don't drop or advance. Emitting an empty response
                    // event here would incorrectly open the response channel
                    // while the model is still producing reasoning, which makes
                    // FoundationModels reject the later reasoning entries.
                    continue
                }
                ...
                emittedDecodedText = decodedText
            }
        ...
        enum ReasoningMode: Equatable, Sendable {
            case standard
            case low
            case disabled
        }

        static func reasoningMode(
            for entries: [Transcript.Entry],
            hasTools: Bool
        ) -> ReasoningMode {
            for entry in entries.reversed() {
                guard case .prompt(let prompt) = entry else { continue }
                let content = textContent(of: prompt.segments)
                if content.contains("/no_think") { return .disabled }
                break
            }
            return hasTools ? .low : .standard
        }
        ...
        private struct ToolCallEntry: Sendable {
            ...
            var message: [String: any Sendable] {
                let argumentObject: [String: any Sendable]
                if let data = arguments.data(using: .utf8),
                    let decoded = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
                {
                    argumentObject = decoded.mapValues { JSONValue($0).sendable }
                } else {
                    argumentObject = [:]
                }
                return [
                    "id": id,
                    "type": "function",
                    "function": ["name": name, "arguments": argumentObject]
        ...
        private func makeTokens(
            ...
            if reasoningMode == .low {
                ...
                // for structured arguments and the result-driven answer, so
                // retain visible reasoning while bounding its default effort.
                templateContext["reasoning_effort"] = "low"
            }
            if reasoningMode == .disabled {
                // Qwen's documented per-turn switch. Rendering the closed
                // prefix in the template is deterministic and keeps a recovery
                // attempt from ending inside another unfinished thought block.
            ...
            do {
                ...
                CLILogger.log(
                    ...
                    component: component)
                // A plain-text fallback changes the semantics of a tool transcript
                // by dropping definitions, calls, and outputs. Fail closed instead.
                if !tools.isEmpty || entries.contains(where: {
        ...
        /// or prematurely closing a tool-response wrapper in the chat template.
        private static func sanitizeTemplateContent(_ text: String) -> String {
            text
                .replacingOccurrences(of: "<|", with: "<\u{200B}|")
                .replacingOccurrences(of: "|>", with: "|\u{200B}>")
                .replacingOccurrences(of: "</tool_response>", with: "</tool\u{200B}_response>")
        }
