    ...
    static func make(
        ...
        case .growing:
            return try GrowingKVCache(
                ...
                configuredMaxCapacity: maxContextLength
            )
        case .chunked:
...
struct GrowingKVCache: CoreAIKVCache {
    ...
    init(
        ...
        // Dynamic descriptors do not expose their torch.export upper bound at
        // runtime. Respect the bundle's configured context limit so exponential
        // growth cannot overshoot a 100K model to 131,072 cache slots.
        self.maxCapacity = maxCapacityFromModel > 0
            ? min(maxCapacityFromModel, configuredMaxCapacity)
            : configuredMaxCapacity
        self.currentCapacity = min(initialCapacity, self.maxCapacity)
